...
interface SpeechInputProps {
  ...
  // Auto-stop after silence (VAD commits transcript, then silence for this duration = auto-stop)
...
const SpeechInput = forwardRef<HTMLDivElement, SpeechInputProps>(
  function SpeechInput(
    ...
    const scheduleAutoStop = useCallback(() => {
      if (!autoStopOnSilenceMs || !hasCommittedRef.current) return

      clearAutoStopTimeout()
      autoStopTimeoutRef.current = setTimeout(() => {
        // Auto-stop if we have committed transcripts
        if (hasCommittedRef.current && transcriptsRef.current.committedTranscripts.length > 0) {
          // Trigger stop
    ...
    }, [autoStopOnSilenceMs, clearAutoStopTimeout, onStop])
    ...
    const scribe = useScribe({
      ...
      onPartialTranscript: (data) => {
        ...
        // Clear auto-stop if user is still speaking
        if (data.text.trim()) {
          clearAutoStopTimeout()
        }
      },
      onCommittedTranscript: (data) => {
        ...
        onChange?.(buildEvent(transcriptsRef.current))
        // Schedule auto-stop after commit (user stopped speaking)
        scheduleAutoStop()
      },
      onError,
      onAuthError,
    ...
    // Store scribe ref for auto-stop
    ...
    }, [getToken, scribe, onStart, microphone, clearAutoStopTimeout])
